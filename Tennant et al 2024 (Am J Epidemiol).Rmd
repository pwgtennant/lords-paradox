---
title: "Lord's Paradox Simulation Code for Tennant et al 2024"
...
This is the simulation and analytical code to accompany the research article entitled, "*Lord's 'paradox' explained: The 50-year warning on the use of 'change scores' in observational data*". The code was prepared by [Peter Tennant](https://medicinehealth.leeds.ac.uk/medicine/staff/815/dr-peter-wg-tennant) (University of Leeds and Alan Turing Institute) during 2022-2023 and has been updated following peer-review with the [American Journal of Epidemiology](https://academic.oup.com/aje) in 2023-2024.

Peter Tennant acts as the guarantor for the accuracy of the work.

The code was run using R 4.3.2 and Python 3.11 but relies on several packages that must be installed for the code to run.

The underlying simulations are performed in `DagSim` framework. This is called and executed within this R notebook using the `reticulate` package. Further details of `DagSim` are available in [Hajj et al 2023](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0284443).
...
1.  In the first stage, two datasets are simulated for the two scenarios (with and without mediator-outcome confounding); these and used to create plots (which form Figure 3 and Supplementary Figure 1 in Tennant et al 2024).
...
#Also load the font that will be used in the plots
...
Target_SD       =  9
Path_X0_LY0_1   =  6
Path_X0_CY_1    =  6/np.sqrt(2.1111111111)
Path_RY0_Y0_1   =  np.sqrt(((Target_SD/0.9)**2-Target_SD**2)/Target_SD**2)
Path_LY0_Y0_1   =  1
Path_LY0_LY1_1  =  0.5
Path_Y0_cY1Y0_1 =  -1
Path_CY_LY1_1   =  0.5*np.sqrt(2.1111111111)
...
def L_weight_change_1(LY0_1, LY1_1):
...
# Measured Latent weight Change (Y1_LY0_1)
...
Y1_Y0_1  = ds.Node(name='weight_change',             function=weight_change_1,             args=[Y0_1, Y1_1])
LY1_LY0_1= ds.Node(name='L_weight_change',           function=L_weight_change_1,           args=[LY0_1, LY1_1])
...
S1_graph = ds.Graph([X0_1,RY0_1,LY0_1,Y0_1,CY_1,RY1_1, LY1_1, Y1_1, Y1_Y0_1, LY1_LY0_1, Y1_LY0_1,LY1_Y0_1], 'S1_graph')
...
Path_X0_M0_2    =  5
Path_X0_LY0_2   =  7.8
Path_X0_CY_2    =  7.8/np.sqrt(1.875)
Path_M0_LY0_2   = -0.36                   
Path_M0_CY_2    = -0.36/np.sqrt(1.875) 
Path_RY0_Y0_2   =  np.sqrt(((Target_SD/0.9)**2-Target_SD**2)/Target_SD**2)
Path_LY0_Y0_2   =  1
Path_LY0_LY1_2  =  0.5
Path_Y0_cY1Y0_2 =  -1
Path_CY_LY1_2   =  0.5*np.sqrt(1.875)
...
# Measured Latent weight Change (Y1_LY0_2)
...
Y1_Y0_2  = ds.Node(name='weight_change',        function=weight_change_2,       args=[Y0_2,Y1_2])
LY1_LY0_2= ds.Node(name='L_weight_change',      function=L_weight_change_2,     args=[LY0_2,LY1_2])
...
# Declare that these nodes belong to a DAG
S2_graph = ds.Graph([X0_2,M0_2,RY0_2,LY0_2,Y0_2,CY_2,RY1_2,LY1_2,Y1_2,Y1_Y0_2, LY1_LY0_2, Y1_LY0_2,LY1_Y0_2], 'S2_graph')
...
scenario1              <- py$S1_data
scenario2              <- py$S2_data
...
First conduct the simulation itself in python
...
s1_list <- py$s1_datasets
s2_list <- py$s2_datasets
...
s1mod1a            <- lm(L_weight_change ~ sex, data = s1_list[[i]])   
s1mod1a_estimate   <- s1mod1a$coefficients[2]

# Scenario 1: change score (latent follow up, measured baseline) 
...
s1mod1c            <- lm(L_M_weight_change ~ sex, data = s1_list[[i]])   
s1mod1c_estimate   <- s1mod1c$coefficients[2]

# Scenario 1: change score (measured follow up, measured baseline) 
s1mod1d            <- lm(weight_change ~ sex, data = s1_list[[i]])   
s1mod1d_estimate   <- s1mod1d$coefficients[2]

# Scenario 2: change score (latent follow up, latent baseline)  
s2mod1a            <- lm(L_weight_change ~ sex, data = s2_list[[i]])   
s2mod1a_estimate   <- s2mod1a$coefficients[2]

# Scenario 2: change score (latent follow up, measured baseline) 
...
s2mod1c            <- lm(L_M_weight_change ~ sex, data = s2_list[[i]])   
s2mod1c_estimate   <- s2mod1c$coefficients[2]

# Scenario 2: change score (measured follow up, measured baseline) 
s2mod1d            <- lm(weight_change ~ sex, data = s2_list[[i]])   
...
# Scenario 1: Latent follow-up, measured baseline
...
s1mod2c            <- lm(follow_up_weight ~ sex + L_baseline_weight, data = s1_list[[i]])
s1mod2c_estimate   <- s1mod2c$coefficients[2]

# Scenario 1: Measured follow-up, measured baseline

s1mod2d            <- lm(follow_up_weight ~ sex + baseline_weight, data = s1_list[[i]])
...
# Scenario 2: Latent follow-up, measured baseline
...
s2mod2c            <- lm(follow_up_weight ~ sex + L_baseline_weight, data = s2_list[[i]])
s2mod2c_estimate   <- s2mod2c$coefficients[2]

# Scenario 2: Measured follow-up, measured baseline

s2mod2d            <- lm(follow_up_weight ~ sex + baseline_weight, data = s2_list[[i]])
...
s1mod4a            <- lm(L_weight_change ~ sex + L_baseline_weight, data = s1_list[[i]])
...
s1mod4c            <- lm(L_M_weight_change ~ sex + L_baseline_weight, data = s1_list[[i]])
s1mod4c_estimate   <- s1mod4c$coefficients[2]

# Scenario 1: measured follow-up, measured baseline

s1mod4d            <- lm(weight_change ~ sex + baseline_weight, data = s1_list[[i]])
s1mod4d_estimate   <- s1mod4d$coefficients[2]

# Scenario 2: Latent follow-up, latent baseline

s2mod4a            <- lm(L_weight_change ~ sex + L_baseline_weight, data = s2_list[[i]])
...
s2mod4c            <- lm(L_M_weight_change ~ sex + L_baseline_weight, data = s2_list[[i]])
s2mod4c_estimate   <- s2mod4c$coefficients[2]

# Scenario 2: measured follow-up, measured baseline

s2mod4d            <- lm(weight_change ~ sex + baseline_weight, data = s2_list[[i]])
s2mod4d_estimate   <- s2mod4d$coefficients[2]

# Model 5: Analysis of follow-up without adjustment

# Scenario 1: Latent follow-up - baseline irrelevant

s1mod5ab            <- lm(L_follow_up_weight ~ sex, data = s1_list[[i]])
s1mod5ab_estimate   <- s1mod5ab$coefficients[2]

# Scenario 1: Measured follow-up - baseline irrelevant

s1mod5cd            <- lm(follow_up_weight ~ sex, data = s1_list[[i]])
s1mod5cd_estimate   <- s1mod5cd$coefficients[2]

# Scenario 2: Latent follow-up - baseline irrelevant

s2mod5ab            <- lm(L_follow_up_weight ~ sex, data = s2_list[[i]])
s2mod5ab_estimate   <- s2mod5ab$coefficients[2]

# Scenario 2: Measured follow-up - baseline irrelevant

s2mod5cd            <- lm(follow_up_weight ~ sex, data = s2_list[[i]])
s2mod5cd_estimate   <- s2mod5cd$coefficients[2]
...
simulation_estimates[nrow(simulation_estimates)+1,]  <- c(s1mod1a_estimate, 
                                                          ...
                                                          s2mod5cd_estimate)
# Drop local vectors
rm(s1mod1a_estimate, s1mod1b_estimate, s1mod1c_estimate, s1mod1d_estimate, s1mod2a_estimate, s1mod2b_estimate, s1mod2c_estimate, s1mod2d_estimate, s1mod4a_estimate, s1mod4b_estimate, s1mod4c_estimate, s1mod4d_estimate, s1mod5ab_estimate, s1mod5cd_estimate, s2mod1a_estimate, s2mod1b_estimate, s2mod1c_estimate, s2mod1d_estimate, s2mod2a_estimate, s2mod2b_estimate, s2mod2c_estimate, s2mod2d_estimate, s2mod3a_estimate,s2mod3b_estimate, s2mod3c_estimate, s2mod3d_estimate, s2mod4a_estimate, s2mod4b_estimate, s2mod4c_estimate, s2mod4d_estimate, s2mod5ab_estimate, s2mod5cd_estimate)
...
model_names                 <- list("S1 - Mod 1a: Unadjusted change-score (latent Y1, latent Y0)",
                                    "S1 - Mod 1b: Unadjusted change-score (latent Y1, measured Y0)",
                                    "S1 - Mod 1c: Unadjusted change-score (measured Y1, measured Y0)",
                                    "S1 - Mod 1d: Unadjusted change-score (measured Y1, measured Y0)",
                                    "S1 - Mod 2a: Adjusted follow-up (latent Y1, latent Y0)",
                                    "S1 - Mod 2b: Adjusted follow-up (latent Y1, measured Y0)",
                                    "S1 - Mod 2c: Adjusted follow-up (measured Y1, latent Y0)",
                                    "S1 - Mod 2d: Adjusted follow-up (measured Y1, measured Y0)",
                                    "S1 - Mod 4a: Adjusted change-score (latent Y1, latent Y0)",
                                    "S1 - Mod 4b: Adjusted change-score (latent Y1, measured Y0)",
                                    "S1 - Mod 4c: Adjusted change-score (measured Y1, latent Y0)",
                                    "S1 - Mod 4d: Adjusted change-score (measured Y1, measured Y0)",
                                    "S1 - Mod 5ab: Unadjusted follow-up (latent)",
                                    "S1 - Mod 5cd: Unadjusted follow-up (measured)",
                                    "S2 - Mod 1a: Unadjusted change-score (latent Y1, latent Y0)",
                                    "S2 - Mod 1b: Unadjusted change-score (latent Y1, measured Y0)",
                                    "S2 - Mod 1c: Unadjusted change-score (measured Y1, latent Y0)",
                                    "S2 - Mod 1d: Unadjusted change-score (measured Y1, measured Y0)",
                                    "S2 - Mod 2a: Adjusted follow-up (latent Y1, latent Y0)",
                                    "S2 - Mod 2b: Adjusted follow-up (latent Y1, measured Y0)",
                                    "S2 - Mod 2c: Adjusted follow-up (measured Y1, latent Y0)",
                                    "S2 - Mod 2d: Adjusted follow-up (measured Y1, measured Y0)",
                                    "S2 - Mod 3a: g-computation (latent Y1, latent Y0)",
                                    "S2 - Mod 3b: g-computation (latent Y1, measured Y0)",
                                    "S2 - Mod 3c: g-computation (measured Y1, latent Y0)",
                                    "S2 - Mod 3d: g-computation (measured Y1, measured Y0)",
                                    "S2 - Mod 4a: Adjusted change-score (latent Y1, latent Y0)",
                                    "S2 - Mod 4b: Adjusted change-score (latent Y1, measured Y0)",
                                    "S2 - Mod 4c: Adjusted change-score (measured Y1, latent Y0)",
                                    "S2 - Mod 4d: Adjusted change-score (measured Y1, measured Y0)",
                                    "S2 - Mod 5ab: Unadjusted follow-up (latent)",
                                    "S2 - Mod 5cd: Unadjusted follow-up (measured)")
