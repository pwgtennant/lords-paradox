#### LORD'S PARADOX SIMULATION CODE FOR TENNANT ET AL 2023 ###############################
#
# Peter WG Tennant - Last updated 2022 02 02
#
...
# The code was prepared by Peter Tennant (University of Leeds and Alan Turing Institute) during 2022-2023. He acts as the guarantor for the accuracy of the work.  
# The code was run using R 4.2.2, but relies on several packages including CMAverse, which may need to be installed from Github.
#
...
#
# NOTE: This code will create a CSV of a simulated dataset, a CSV containing the results, and a PNG of the Lord's Paradox plot in the working directory
#
...
#### MAKE LORD'S PARADOX FIGURE ###############################
#
# This section focuses on creating the Lord's Paradox plot from a single simulated dataset of 10000 units
...
data_dictionary = DAG.simulate(num_samples=10000, csv_name='lordsimulation')


")

# Import the simulated data ----
lordsimulation              <- read.csv('lordsimulation.csv')

# Transform variables to have plausible means and SDs ----
lordsimulation              <- lordsimulation %>% 
...
# The plot will be constructed from four panels, made up of two density plots of the baseline weight and follow-up weight distributions, the scatterplot of follow-up vs baseline weight, and a blank square 

# Draw the first density plot, for baseline weight ----

left_histogram              <- ggplot(lordsimulation) +
                                      ...
                                      geom_density(data=subset(lordsimulation, sex=="Female"), 
                                                   ...
                                                   linewidth=1, 
                                                   alpha=0.1) +
                                      geom_density(data=subset(lordsimulation, sex=="Male"), 
                                                   aes(x=follow_up_weight, y=-after_stat(count)),
                                                   bw=2.5, 
                                                   fill="#64197D", 
                                                   color="#64197D", 
...
bottom_histogram            <- ggplot(lordsimulation) +
                                      theme_classic(base_size = 16, 
                                                    base_family="Roboto") +
                                      theme(legend.position = "none",
...
scatter                     <- ggplot(lordsimulation, aes(x = baseline_weight, y = follow_up_weight)) +
                                      ...
                                      theme(legend.position  = "none",
                                            ...
                                            axis.title.y     = element_blank(),
                                            text             = element_text(family = "Roboto")) +
                                      geom_point(data=subset(lordsimulation, sex=="Female"), 
                                                 size=0.5, 
                                                 colour="#C83296", 
                                                 alpha=0.5, 
                                                 shape=16) +
                                      geom_point(data=subset(lordsimulation, sex=="Male"), 
                                                 size=0.5, 
                                                 colour="#009632", 
                                                 alpha=0.5, 
                                                 shape=16) +
                                      ...
                                      geom_point(data=subset(lordsimulation, sex=="Female"), 
                                                 ...
                                                 colour="#C83296", 
                                                 shape=16) +
                                      stat_ellipse(data=subset(lordsimulation, sex=="Female"), 
                                                   colour="#C83296", 
                                                   linetype="longdash", 
                                                   linewidth=1, 
                                                   level = 0.995) +
                                      ...
                                      geom_point(data=subset(lordsimulation, sex=="Male"), 
                                                 ...
                                                 colour="#009632", 
                                                 shape=16) +
                                      stat_ellipse(data=subset(lordsimulation, sex=="Male"), 
                                                   colour="#009632", 
                                                   linetype = "solid", 
                                                   linewidth=1, 
                                                   level = 0.995) +
                                      stat_ellipse(data=subset(lordsimulation, sex=="Male"), 
                                                   ...
                                                   fill="#009632", 
                                                   level = 0.995) +
                                      geom_smooth(data=subset(lordsimulation, sex=="Female"),
                                                  method="lm", 
                                                  se = FALSE, 
                                                  colour="#C83296", 
                                                  linetype="dashed", 
                                                  fullrange=TRUE) + 
                                      geom_smooth(data=subset(lordsimulation, sex=="Male"),
                                                  method="lm", 
                                                  se = FALSE, 
                                                  colour="#009632", 
                                                  linetype="solid", 
...
Lord_plot <- grid.arrange(left_histogram, scatter, empty, bottom_histogram, 
...
ggsave(paste0("Lord_plot", ".png"), plot = Lord_plot, units = "px", height = 3000, width = 3000, dpi=300)

#### EFFECT ESTIMATES AND COEFFICIENTS ###############################
#
# This section focuses on estimating and saving the results of various models 
...
simulation_estimates        <- data.frame(mod1=numeric(0),
                                          mod2=numeric(0),
                                          mod3=numeric(0),
                                          mod4=numeric(0), 
                                          mod5=numeric(0),
...
mod1            <- lm(weight_change ~ sex,           data = lordsimulation)   
mod1_estimate   <- mod1$coefficients[2]

# LOOP: Model 2: Analysis of follow-up adjusted for baseline ----
mod2            <- lm(follow_up_weight ~ sex + baseline_weight,      data = lordsimulation)
mod2_estimate   <- mod2$coefficients[2]

# LOOP: Model 3 & 6: Mediation analysis ----
invisible(capture.output(
mod3            <- cmest(data = lordsimulation,
                         model="gformula",
                         exposure = "sex",
...
mod3_estimate   <- mod3$effect.pe[1]
mod6_estimate   <- mod3$effect.pe[6]

# LOOP: Model 4: Analysis of change-score adjusted for baseline ----
mod4            <- lm(weight_change ~ sex + baseline_weight, data = lordsimulation)
mod4_estimate   <- mod4$coefficients[2]

# LOOP: Model 5: Analysis of follow-up without adjustment ----
mod5            <- lm(follow_up_weight ~ sex,      data = lordsimulation)
mod5_estimate   <- mod5$coefficients[2]

# LOOP: Save local vectors into dataframe ----

simulation_estimates[nrow(simulation_estimates)+1,]  <- c(mod1_estimate, mod2_estimate, mod3_estimate, mod4_estimate, mod5_estimate, mod6_estimate)

# Drop local vectors
rm(mod1_estimate, mod2_estimate, mod3_estimate, mod4_estimate, mod5_estimate, mod6_estimate)
...
model_names                 <- list("mod1","mod2", "Mod3","mod4","mod5","mod6")
